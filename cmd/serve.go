...
var serveCmd = &cobra.Command{
	Use:   "serve",
	Short: "serves the REST Server",
	Long:  `This subcommand serves the REST Server`,
...
func init() {
	...
	serveCmd.PersistentFlags().IntVar(&argInsecurePort, "insecure-port", 9091, "The port to listen to for incoming HTTP requests.")
	serveCmd.PersistentFlags().IntVar(&argPort, "port", 8443, "The secure port to listen to for incoming HTTPS requests.")
	serveCmd.PersistentFlags().IPVar(&argInsecureBindAddress, "insecure-bind-address", net.IPv4(127, 0, 0, 1), "The IP address on which to serve the --port (set to 0.0.0.0 for all interfaces).")
	serveCmd.PersistentFlags().IPVar(&argBindAddress, "bind-address", net.IPv4(0, 0, 0, 0), "The IP address on which to serve the --secure-port (set to 0.0.0.0 for all interfaces).")
	serveCmd.PersistentFlags().StringVar(&argDefaultCertDir, "default-cert-dir", "/certs", "Directory path containing '--tls-cert-file' and '--tls-key-file' files. Used also when auto-generating certificates flag is set.")
	serveCmd.PersistentFlags().StringVar(&argCertFile, "tls-cert-file", "", "File containing the default x509 Certificate for HTTPS.")
	serveCmd.PersistentFlags().StringVar(&argKeyFile, "tls-key-file", "", "File containing the default x509 private key matching --tls-cert-file.")
	serveCmd.PersistentFlags().StringVar(&argApiserverHost, "apiserver-host", "", "The address of the Kubernetes Apiserver "+
		"to connect to in the format of protocol://address:port, e.g., "+
		"http://localhost:8080. If not specified, the assumption is that the binary runs inside a "+
		"Kubernetes cluster and local discovery is attempted.")
	serveCmd.PersistentFlags().StringVar(&argHeapsterHost, "heapster-host", "", "The address of the Heapster Apiserver "+
		"to connect to in the format of protocol://address:port, e.g., "+
	...
	serveCmd.PersistentFlags().StringVar(&argKubeConfigFile, "kubeconfig", "", "Path to kubeconfig file with authorization and master location information.")
	serveCmd.PersistentFlags().IntVar(&argTokenTTL, "token-ttl", int(authApi.DefaultTokenTTL), "Expiration time (in seconds) of JWE tokens generated by dashboard. Default: 15 min. 0 - never expires")
	serveCmd.PersistentFlags().StringSliceVar(&argAuthenticationMode, "authentication-mode", []string{authApi.Token.String()}, "Enables authentication options that will be reflected on login screen. Supported values: token, basic. Default: token."+
		"Note that basic option should only be used if apiserver has '--authorization-mode=ABAC' and '--basic-auth-file' flags set.")
	serveCmd.PersistentFlags().IntVar(&argMetricClientCheckPeriod, "metric-client-check-period", 30, "Time in seconds that defines how often configured metric client health check should be run. Default: 30 seconds.")
	serveCmd.PersistentFlags().BoolVar(&argAutoGenerateCertificates, "auto-generate-certificates", false, "When set to true, Dashboard will automatically generate certificates used to serve HTTPS. Default: false.")
	serveCmd.PersistentFlags().BoolVar(&argEnableInsecureLogin, "enable-insecure-login", false, "When enabled, Dashboard login view will also be shown when Dashboard is not served over HTTPS. Default: false.")
	serveCmd.PersistentFlags().StringVar(&argSystemBanner, "system-banner", "", "When non-empty displays message to Dashboard users. Accepts simple HTML tags. Default: ''.")
	serveCmd.PersistentFlags().StringVar(&argSystemBannerSeverity, "system-banner-severity", "INFO", "Severity of system banner. Should be one of 'INFO|WARNING|ERROR'. Default: 'INFO'.")
	serveCmd.PersistentFlags().StringVar(&argsAzureApplicationId, "azure-application-id", "f145fb87-901d-4519-ad39-573c30770e6f", "")
	serveCmd.PersistentFlags().StringVar(&argsAzureRedirect, "azure-redirect", "http://localhost:4326", "")
	serveCmd.PersistentFlags().StringVar(&argsAzureScope, "azure-scope", "User.ReadBasic.All+Contacts.Read+Mail.Send+Files.ReadWrite+Notes.ReadWrite", "")
	serveCmd.PersistentFlags().StringVar(&argsKuberPlane, "kuber-plane", "http://localhost:4326", "")
	serveCmd.PersistentFlags().StringVar(&argsKuberRest, "kuber-rest", "http://localhost:9091", "")
	serveCmd.PersistentFlags().StringVar(&argsKuberWs, "kuber-ws", "ws://localhost:9091", "")
	serveCmd.PersistentFlags().StringVar(&argsSpitfireRest, "spitfire-rest", "http://localhost:8091", "")
	serveCmd.PersistentFlags().StringVar(&argsSpitfireWs, "spitfire-ws", "ws://localhost:8091", "")
	serveCmd.PersistentFlags().StringVar(&argsHdfs, "hdfs", "http://localhost:50070", "")
	serveCmd.PersistentFlags().StringVar(&argsTwitterRedirect, "twitter-redirect", "http://datalayer-001:8081/twitter/maketoken", "")

	//	viper.BindPFlags(pflag.CommandLine)
	viper.BindPFlag("azureapplicationid", serveCmd.PersistentFlags().Lookup("azure-application-id"))
	viper.BindPFlag("azureredirect", serveCmd.PersistentFlags().Lookup("azure-redirect"))
	viper.BindPFlag("azurescope", serveCmd.PersistentFlags().Lookup("azure-scope"))
	viper.BindPFlag("kuberplane", serveCmd.PersistentFlags().Lookup("kuber-plane"))
	viper.BindPFlag("kuberrest", serveCmd.PersistentFlags().Lookup("kuber-rest"))
	viper.BindPFlag("kuberws", serveCmd.PersistentFlags().Lookup("kuber-ws"))
	viper.BindPFlag("spitfirerest", serveCmd.PersistentFlags().Lookup("spitfire-rest"))
	viper.BindPFlag("spitfirews", serveCmd.PersistentFlags().Lookup("spitfire-ws"))
	viper.BindPFlag("hdfs", serveCmd.PersistentFlags().Lookup("hdfs"))
	viper.BindPFlag("twitterredirect", serveCmd.PersistentFlags().Lookup("twitter-redirect"))
